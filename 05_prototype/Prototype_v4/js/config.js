...
const STUFEN = [
  null,
  { label: 'Lernbedarf',    color: '#8B1A1A', bg: '#FEE2E2' },
  { label: 'Grundlagen',    color: '#7A4A00', bg: '#FEF3C7' },
  { label: 'Verstanden',    color: '#A16207', bg: '#FEF9C3' },
  { label: 'Klausurniveau', color: '#1A5C3A', bg: '#D1FAE5' },
  { label: 'Exzellent',     color: '#4A2C6E', bg: '#EDE6F5' },
]

// ─── Score System (v4) ────────────────────────────────────────────────────────
// Stufe → Score mapping (used when converting old diagnose data or completing diagnose)
const STUFE_TO_SCORE = { 1: 10, 2: 30, 3: 50, 4: 75, 5: 92 }

// Score deltas applied after each task result
const SCORE_DELTA = {
  correct_0tips: +8,
  correct_1tip:  +3,
  correct_2tips: -2,
...
// Klausurniveau target
const KLAUSUR_SCORE = 75
const KLAUSUR_AXIS  = 3.75  // = 75 / 20 (radar axis 0–5)

/**
 * Returns visual config for a given score (0–100) or null.
 */
function scoreConfig(score) {
  if (score === null || score === undefined) return { bg: '#F3F4F6', color: '#7A6E8A' }
  if (score < 20) return { bg: '#FEE2E2', color: '#8B1A1A' }
  if (score < 40) return { bg: '#FEF3C7', color: '#7A4A00' }
  if (score < 60) return { bg: '#FEF9C3', color: '#A16207' }
  if (score < 80) return { bg: '#D1FAE5', color: '#1A5C3A' }
  return { bg: '#EDE6F5', color: '#4A2C6E' }
}
